...
_data = map(lambda l: l.split(','),_data.split('\n'))


def draw(start, direction, length, id, wire_length, record, min_signal_length):
    (x, y) = start
    ...
    for i in range(length):
        ...
        if (x, y) not in record:
            record[(x, y)] = {}
            record[(x,y)][id] = wire_length
        elif id not in record[(x, y)]:
            record[(x,y)][id] = wire_length
            min_signal_length = min(min_signal_length, sum(record[(x, y)].values()))
            # min_distance = min(min_distance, abs(x) + abs(y))
    return (x, y), wire_length, min_signal_length
...
def print_record(record):
    ...
    for y in range(min_y, max_y + 1):
        for x in range(min_x, max_x + 1):
            if (x,y) not in record or len(record[(x,y)]) == 0:
                s += '.'
            elif len(record[(x,y)]) == 1:
                s += 'O'
            else:
                s += 'X'
...
_record = dict()
for line in _data:
    _min_signal_length = math.inf
    _wire_length = 0
    _id += 1
    _x, _y = 0, 0
    for segment in line:
        _direction, _length = parse(segment)
        (_x, _y), _wire_length, _min_signal_length = draw((_x, _y), _direction, _length, _id, _wire_length, _record, _min_signal_length)
        # print_record(_record)
print(_min_signal_length)
